...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,getTokenHeader:function(){
          var obj = layui.data(layui.setter.tableName);
          var headers = {};
          headers[layui.setter.request.tokenName] = obj[layui.setter.request.tokenName];
          return headers;
    }
    ,checkUserLogin:function(){
        var obj = layui.data(layui.setter.tableName);
        var token = obj[layui.setter.request.tokenName];
        if(!token){
          this.topestFrameHref(window,layui.setter.basePath + '/login.html');
        }
    }
    ,topestFrameHref:function(win,url){
      if (win.top != null && win.top.location == win.location) {
          win.location.href = layui.setter.basePath + '/login.html';
      }else{
          this.topestFrameHref(win.top,url);
      }
    }
  };
